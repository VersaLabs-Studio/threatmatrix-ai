...
const FLOW_COLS = [
  {
    ...
    header: "Time",
    width: 90,
    render: (r: NetworkFlow) => formatTime(r.timestamp),
  },
  ...
  { key: "protocol", header: "Proto", width: 60 },
  {
    ...
    render: (r: NetworkFlow) =>
      formatBytes((r.src_bytes ?? 0) + (r.dst_bytes ?? 0)),
  },
  {
    key: "anomaly_score",
    header: "ML Score",
    ...
    render: (r: NetworkFlow) => {
      ...
      return (
        <span
          style={{ color, fontFamily: "var(--font-data)", fontWeight: 700 }}
        >
...
export default function NetworkFlowPage() {
  ...
  const { flows, stats, protocols, topTalkers, loading } = useFlows({
    time_range: "1h",
    limit: 100,
  });
  ...
  const filtered = allFlows.filter((f) => {
    ...
    if (search && !f.src_ip.includes(search) && !f.dst_ip.includes(search))
      return false;
    return true;
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <MetricCard
          label="TOTAL BYTES"
          value={allFlows.reduce(
            (s, f) => s + (f.src_bytes ?? 0) + (f.dst_bytes ?? 0),
            0,
          )}
          unit="B"
          accent="info"
      ...
      <GlassPanel
        ...
        icon="📋"
        title="FLOW TABLE"
        badge={`${filtered.length} / ${allFlows.length}`}
      >
        {/* Filters row */}
        <div
          style={{
                ...
                style={{
                  ...
                  background:
                    protoFilter === p
                      ...
                      : "var(--bg-tertiary)",
                  color:
                    protoFilter === p ? "var(--cyan)" : "var(--text-muted)",
                  cursor: "pointer",
                }}
              >
                {p}
